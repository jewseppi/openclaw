 ...
 *
 * The shared-state worker admits only the host thread, so a test that drains or
 * persists a queue must run in a forked pool: such suites belong in
 * `databaseWorkerCoreTestFiles`, not a default `threads` shard.
 */
